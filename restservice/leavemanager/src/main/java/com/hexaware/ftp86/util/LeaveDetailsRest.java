...
public class LeaveDetailsRest {
 ...
 /**
  ...
  @POST
  @Path("/updateleave/{empId}")
  @Consumes(MediaType.APPLICATION_JSON)
  @Produces(MediaType.APPLICATION_JSON)
  public final String employeeupdateLeave(@PathParam("empId") final int empId,
                   final LeaveDetails ld) throws ParseException {
    SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");
    String leType = ld.getLeaveType().toString();
    String result = LeaveDetails.updateLeave(empId, sdf.format(ld.getLeaveSdate()), sdf.format(ld.getLeaveEdate()),
                          ld.getLeaveNdays(), leType, ld.getLeaveReason(), ld.getLeaveId());
    return result;
  }
}

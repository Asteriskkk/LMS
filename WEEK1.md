...
IdentityFile ~/.ssh/id_rsa
  ...
  ** Clone repository at Gitbash **
    ...
    * `pwd` -- Check and verify that the current working directory is C:\users\Hvuser/workspace
    * `git clone ssh://git-codecommit.us-east-2.amazonaws.com/v1/repos/ftp86 `
    * `cd ftp86`
    * `git status`
  * open Visual Studio (VS) Code
    * Open folder c:\users\Hvuser\workspace\ftp86
...
# Workshop material - Day #2 - MySQL Workbench/Cli/Curl/Browser to interact w/ application
  ...
  * `CREATE DATABASE ftp86;` and click the lightning button
  * `CREATE USER 'ftp86'@'localhost' IDENTIFIED BY 'ftp86';`
  * `GRANT ALL ON ftp86.* TO 'ftp86'@'localhost';`
  ...
  * go to gitbash, ensure you are in workspace/ftp86
  * `cd restservice/leavemanager`
  * `mvn compile`
  * `mvn exec:java -Dexec.mainClass=com.hexaware.ftp86.util.CliMain`
  ...
  * Build the war file as follows:
    ...
    * `cp target/ftp86-0.0.1-SNAPSHOT.war /D/FTP/apache-tomcat-8.5.16-windows-x64/apache-tomcat-8.5.16/webapps/ftp86.war`
    * start tomcat and tail its logs
      * `cd D/FTP/apache-tomcat-8.5.16-windows-x64/apache-tomcat-8.5.16`
      * `rm -rf logs/*`
      * `./bin/startup.sh`
      * `tail -f logs/*`
      * `curl -vvv http://localhost:8080/ftp86/api/employees | python -m json.tool`
      * `curl -vvv http://localhost:8080/ftp86/api/employees/2000 | python -m json.tool`
   * `cd ../../webui/lm-app/`
   * `npm install`
   * `ng build`
   * `cp ../../restservice/leavemanager/target/ftp86-0.0.1-SNAPSHOT.war ./ftp86.war`
   * `cd dist`
   * `jar -uvf ../ftp86.war *`
   * `cd ..`
   * `jar -tvf ftp86.war`
   * `cp ftp86.war /D/FTP/apache-tomcat-8.5.16-windows-x64/apache-tomcat-8.5.16/webapps/ftp86.war`
   * Notice that the tail terminal shows that the new version of the web application archive is now getting deployed
   * Open Chrome browser and navigate to http://localhost:8080/ftp86/
...
# Workshop material - Day #4 - Configure Project Board in Trello for  Managing and Tracking Your Project
  ...
  * Create a board for your project as "ftp86"
  ...
  * [Team member #3]
     * Create "Current Sprint" with "ftp86 Sprint 1" as label 
